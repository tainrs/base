...
ENV IMAGE_STATS=${IMAGE_STATS} BUILD_ARCHITECTURE=${BUILD_ARCHITECTURE} \
    ...
    TS_ENABLED="false" TS_HOSTNAME="" TS_ADVERTISE_ROUTES="" TS_ACCEPT_ROUTES="" TS_STATE_DIR="${CONFIG_DIR}/tailscale" TS_AUTHKEY="" TS_NO_RESET="" \
    TS_ACCEPT_DNS="" TS_ADVERTISE_EXIT_NODE="" TS_FORCE_REAUTH="" TS_SOCKET="${CONFIG_DIR}/tailscale/tailscaled.sock" TS_ADVERTISE_TAGS="" TS_SERVE="32400" TS_SNAT_SUBNET_ROUTES="false"
...
RUN set -e ;\
    ...
    apt-get install -y --no-install-recommends --no-install-suggests \
        net-tools iputils-ping ca-certificates curl dos2unix figlet jq locales python3 libcap2-bin tzdata unzip wget xz-utils ;\
    # Install Tailscale and Unbound
